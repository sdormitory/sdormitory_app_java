...
public class SysUserController {
    ...
    public CommonResult<List<SysUser>> listAll() {
        List<SysUser> users = sysUserService.getListAll();
        return CommonResult.success(users);
    }

    @ApiOperation("listAll => 获取所有有效班主任列表")
    @GetMapping(value = "/getClassTeacherList")
    public CommonResult<List<SysUser>> getClassTeacherList() {
        List<SysUser> users = sysUserService.getClassTeacherList();
        return CommonResult.success(users);
    }

    @ApiOperation("listAll => 获取所有有效宿管老师列表")
    @GetMapping(value = "/getDorTeacherList")
    public CommonResult<List<SysUser>> getDorTeacherList() {
        List<SysUser> users = sysUserService.getDorTeacherList();
        return CommonResult.success(users);
    }
